...
export default function AdminSharedReadingsPage() {
    ...
    return (
        <AdminLayout activePath="/admin/shared-readings" breadcrumb="Shared Readings" username={user.username ?? "Admin"}>
            <div className="view">
                <PageHeader kicker="Engagement" title="Shared readings" subtitle="Public reading links — what's circulating in the world." />
                ...
                <Table
                    ...
                    renderRow={(r: AdminSharedReading) => (
                        <tr key={r.id}>
                            <td>
                                <div className="cell-reading-title">{r.title || "Untitled reading"}</div>
                                <div className="cell-reading-sub">Shared {new Date(r.created_at).toLocaleDateString()}</div>
                            </td>
                            ...
                            <td>
                                {r.is_public
                                    ? <Pill tone="info"><Icon name="globe" size={11} />Public</Pill>
                                    : <Pill tone="neutral"><Icon name="lock" size={11} />Unlisted</Pill>}
                            </td>
